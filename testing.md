...
```bash
python -m pytest -q              # 251 tests, about 4 seconds
...
| `test_waste_never_exceeds_the_proved_bound` | measured waste stays inside `m*(2d-1)` across the grid |
...
`python eval_protocol.py` writes `summary.csv` (150 cells, each the worst case
